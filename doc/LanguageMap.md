...
The Kusto Query Language was created as part of the Kusto service also known as Azure Data Explorer and the content of this documentation folder matches exactly the language implementation of the Kusto service. The language was opened sourced so that all other data platforms and services can benefit their users with a simple and productive language and to allow their users whether they are developers, data scientists or data consumers who are familiar with the Kusto Query Language to leverage their language skills across wide variety of products and services.

To achieve this goal, it is essential that the following conditions will be met:

* The semantic of the language is the same across all products and services so that for a given data set the same query will return the same results.
* The supported flavor of the language is known and well defined.

Since the original full language will probably not be applicable to all product and services, we categorized the language into flavors, and recommend that the implementer of the language will pick one of the flavors so that users can easily know what are the language constructs each product or service supports.
