...
namespace Kusto.Language.Parsing
{
    ...
    public static class SafeScanner
    {
        ...
        {
            var sss = StackSafeScanner<TInput>.pool.AllocateFromPool();
            try
            {
            ...
            {
                StackSafeScanner<TInput>.pool.ReturnToPool(sss);
            }
        }

        private class StackSafeScanner<TInput> : ParserVisitor<TInput, Parser<TInput>>
        {
            ...
            public static readonly ObjectPool<StackSafeScanner<TInput>> pool =
                new ObjectPool<StackSafeScanner<TInput>>(() => new StackSafeScanner<TInput>(), s => s.Clear());

            public StackSafeScanner()
            {
