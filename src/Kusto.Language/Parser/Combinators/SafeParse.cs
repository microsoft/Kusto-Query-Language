...
namespace Kusto.Language.Parsing
{
    ...
    public static class SafeParser
    {
        ...
        {
            var ssp = StackSafeParser<TInput>.pool.AllocateFromPool();
            try
            {
            ...
            {
                StackSafeParser<TInput>.pool.ReturnToPool(ssp);
            }
        }

        private class StackSafeParser<TInput> : ParserVisitor<TInput, Parser<TInput>>
        {
            ...
            public static readonly ObjectPool<StackSafeParser<TInput>> pool =
                new ObjectPool<StackSafeParser<TInput>>(() => new StackSafeParser<TInput>(), p => p.Clear());

            public StackSafeParser()
            {
