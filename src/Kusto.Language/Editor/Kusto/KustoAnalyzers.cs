...
namespace Kusto.Language.Editor
{
    ...
    public static class KustoAnalyzers
    {
        ...
        /// </remarks>
        public static readonly KustoAnalyzer PreferUsingMaterializedViewIntrinsic = KustoAnalyzer.Create(
            nameof(PreferUsingMaterializedViewIntrinsic),
            new Diagnostic(
                "KS511",
                DiagnosticCategory.Performance,
                DiagnosticSeverity.Suggestion,
            ...
            (code, dx, diagnostics) =>
            {
                if (code.Kind == CodeKinds.Query)
                {
                    foreach (var nameReference in code.Syntax.GetDescendants<NameReference>())
                    {
                        ...
                        {
                            AnalyzeMaterializedViewSymbol(nameReference, materializedViewSymbol, code, dx, diagnostics);                           
                        }
                    }
                }
            });
