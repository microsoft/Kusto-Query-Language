...
namespace Kusto.Language.Editor
{
...
#else
            AnnotationFinder<LexicalToken>.FindAnnotations(source, matchIndex, grammar, startIndex, annotatedParsers);
#endif
        }
    ...
    internal class AnnotationFinder<TInput> : ParserVisitor<TInput, int, int>
    {
        ...
        /// <param name="root">The root/start of the grammar</param>
        /// <param name="start">The starting offset associated with the root grammar</param>
        /// <param name="annotatedParsers">The output list of annotated parsers.</param>
        public static void FindAnnotations(Source<TInput> source, int findOffset, Parser<TInput> root, int start, List<Parser<TInput>> annotatedParsers)
        {
            var finder = new AnnotationFinder<TInput>(source, findOffset, annotatedParsers);
